...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body >
        ...
        <form method="post" enctype="multipart/form-data" action="excel">
            ...
            @csrf
             <input name="tarub" type="file" @change="uploadFile" ref="file">
            <input @click.prevent="submitPrepareExcel" type="submit">
  ...
  createApp({
    ...
    console.log(this.file)
     ...
     async submitPrepareExcel() {
        ...
        await axios.post('http://excel.test/excel', formData, { headers }).then((res) => {
      ...
      async submitUploadExcel(eskapo) {
       var obj = []
       var modHeader = this.modelHeader.map((e) => {
        if (eskapo.target.elements[e].value !== '') { 
            obj.push({ [e] : eskapo.target.elements[e].value })
          }
       })
        ...
        const headers = { 'Content-Type': 'multipart/form-data' };
        await axios.post('http://excel.test/upload', formData, { headers }).then((res) => {
